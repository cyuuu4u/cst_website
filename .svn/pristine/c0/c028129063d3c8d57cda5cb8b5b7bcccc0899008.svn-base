...
public class DocumentAction extends Action2 {
		...
		if (documentKindId == -1) {
 		 ...
 		 if(true) {
			try {
				...
				po.setDesc(this.documentForm.getBrief());
				po.setContont(this.documentForm.getArticle());
	...
	public String showDocumentByKindId(){
		try {
			...
			{
			     listDocument = this.documentService.findByDocumentKindId(documentKindId);
			}
			jsonVo.setSuccess(true);
			jsonVo.setRows(listDocument);
			jsonVo.setTotal(listDocument.size());
	...
	public String uploadImg() {
		...
		if(uploadImgtry()) {
			try {
				...
				//String filename=getUpload().getActualFilename("upload");
