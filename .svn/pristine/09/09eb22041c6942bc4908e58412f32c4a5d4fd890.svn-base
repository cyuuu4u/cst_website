...
public class DocumentDao extends DaoBase {
	...
	public void add1(Document po) throws SQLException {
		...
		listValue.add(po.getContont());
		listValue.add(po.getDesc());
		String strSql = "INSERT INTO t_document (f_name, f_documentKindId, f_uploadTime, f_lastEditTime, f_creatorId, f_contont, f_desc) VALUES ";
		strSql += "(?, ?, now(), now(), ?, ?, ?)";
	...
	public List<Document> findById(Integer id) throws SQLException {
		List<Document> document=new ArrayList<Document>();
		String strSql="SELECT * FROM t_document WHERE f_id="+id;
		document= DbUtil.executeQuery(strSql, Document.class);
		return document;
	}
	...
	public List<Document> findNewsByDocumentKindId(Integer documentKindId) throws SQLException {
		...
		{
			KindName=kind.getName();
			strSql="SELECT * FROM t_document WHERE f_documentKindId="+kind.getCode()+" ORDER BY f_id DESC ";
			List<Document> document2=new ArrayList<Document>();
			document2= DbUtil.executeQuery(strSql, Document.class);
			for(Document docu:document2)
			{
